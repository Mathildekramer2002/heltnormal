...
eller er pårørende til en, med en psykisk diagnose. Disse skal være med til at 
normalisere psykiske sygdomme, samt nedbryde fordomme og stigma.
...
som alle gruppemedlemmer blev inviteret til, kunne vi arbejde i den samme kode, ved at push og pull.
...
forskellige teknikker. Vi skiftede mellem hvem der gjorde det, så vi alle fik lov at 
sidde med det, og få en bedre forstålelse, men selvom kun én person fysisk skrev koden ad 
...
Det gjorde det muligt at udvikle flere dele af projektet samtidig, samtidig med 
at vi fortsat koordinerede vores arbejde gennem fælles beslutninger og løbende 
...
Så forklaret hvad det er vi har lavet/ændret, samt hvad det betyder for løsning, 
det vi enten har tilføjet eller ændret. 

## Indsigter fra github
Her ses screenshots fra indsigter på GitHub. Her ses det hvordan vi har samarbejdet, 
...
<p>
 ...
 ### Mellemrum & (æ,ø,å)
 - Vi har undgået mellemrum samt (æ, ø & å) i vores filnavne. Dette har vi gjort for, 
 at reducere risikoen for fejl, og for at undgå problemer med filstier og links mellem filerne.

 ### Små bogstaver

- Vi har kun brugt små bogstaver i vores fil- og mappenavne. Det gør navngivningen mere 
ensartet og gør det lettere at linke mellem filerne. 
Eksempler fra projektet er `index.html`, `statistik.html` og `myterogfakta.js`.

### Mappestruktur

- Vi har organiseret vores filer i mapper, så projektet er nemmere at navigere i. 
Vi har blandt andet en mappe med vores css-fil, en mappe til alle vores JavaScript-filer 
og en mappe der hedder "image", hvor alle vores billeder ligger i, 
samt en mappe med underteksterne til vores videoer:
...
hvad funktionen gør eller hvad variablen indeholer.
...
Her gør navnene på vores variabler det lettere at forstå, at `videreBtn` henviser 
til videre-knappen, `udsagnListe` indeholder quizspørgsmålene, og `aktueltSporgsmaal` holder styr på, 
...
fx `videreBtn`, `udsagnListe` og `visSporgsmaal()`. Det betyder, at det første ord 
starter med lille bogstav, mens de efterfølgende ord starter med stort bogstav. 
Det gør navnene lettere at læse, da de forskellige ord bliver tydeligt adskilt uden brug af mellemrum.

# Kode eksempel
### Undertekster på videosiden

Vi har valgt at forklare koden, som gør det muligt for brugeren, 
at slå undertekster til og fra, på videoerne man finder på siden personlige historier.
...
om underteksterne er slået til eller fra. Variablen er sat til `false` fra start, 
hvilket betyder, at underteksterne er skjult, når videoen åbnes.

Herefter bruger vi `undertekstBtn.addEventListener("click")`, som lytter efter klik 
på undertekst-knappen. Når brugeren klikker på knappen, ændres værdien 
...
Hvis de er det, bliver CSS-klassen `aktiv` tilføjet, ved hjælpe af `classList.add()`. 
Dette giver brugeren visuel feedback, så de tydeligt kan se, at funktionen er slået til.

i `else` fjernes klassen igen med `classList.remove()`, og undertekstfeltet 
skjules med `display: none`, så teksten ikke vises på skærmen længere. 
...
Vi undersøger om underteksterne er slået til, og om der findes en aktiv undertekst, 
på det aktuelle tidspunkt af videoen. Hvis begge betingelser er opfyldt, 
så indsættes teksten i vores eget undertekstfelt, ved hjælp af `customUndertekst.textContent`, 
og feltet bliver vist på skærmen.

Hvis der ikke findes en aktiv undertekst, eller hvis brugeren har slået underteksterne fra, 
så skjules undertekstfeltet igen. 

Vi har valgt denne løsning for at gøre de personlige fortællinger mere tilgængelige. 
Nogle brugere har lettere ved at følge historien, når de både kan høre og læse indholdet. 
...
vi anvender i vores JavaScript-kode. På den måde bruges dataene til dynamisk at generere 
indholdet på siden ved hjælp af JavaScript, så nye historier kan tilføjes uden at ændre HTML-strukturen.
...
Hver historie er repræsenteret som et objekt, hvor attributterne fra ORCA-tabellen, 
er blevet omsat til properties med forskellige datatyper. 
...
kan have værdien `true` eller `false`. Vi har derfor brugt den til de funktionerne, 
der kun har to mulige tilstande, som fx vores undertekster. 
...
om underteksterne på videoen er slået til eller fra. Variablen starter med værdien `false`, 
hvilket betyder, at underteksterne er skjult når brugeren åbner videoen. 
Når brugeren klikker på undertekst-knappen, ændres værdien til det modsatte ved hjælp af `!`. 
Det betyder altså, at hvis værdien er `true`, 
...
# Kommentarer i koden
Vi har anvendt kommentarer i alle filer, så både HTML, JavaScript og CSS. 
Dette har vi gjort for holde koden overskuelig, samt lettere for os at forstå. 
